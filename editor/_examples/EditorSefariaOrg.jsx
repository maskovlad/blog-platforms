...
const replaceDivineNames = (str, divineName) => {
  ...
  // Currently ignores ЧђЦµЧњ & Ч¦Ц°Ч‘ЦёЧђЧ•Ц№ЧЄ & Ч©Ч“Ч™
  const divineRE = /([\s.,\u05BE;:'"\-]|^)([Ч•]?[\u0591-\u05C7]*[ЧћЧ©Ч”Ч•Ч›ЧњЧ‘]?[\u0591-\u05C7]*)(Ч™[\u0591-\u05C7]*Ч”[\u0591-\u05C7]*Ч•[\u0591-\u05C7]*Ч”[\u0591-\u05C2\u05C4-\u05C7]*|Ч™Ц°Ч™Цё|Ч™Ч™|Ч™Ч§Ч•Ч§|Ч”\'|Ч”Чі)(?=[/(/[<//.,;:Чѓ'"\-\s]|$)/g;

  // don't match ЧђЦІЧ“Ц№Ч ЦґЧ™
  const adoshemRE = /([\s.,\u05BE;:'"\-]|^)([Ч•]?[\u0591-\u05C7]*[ЧћЧ©Ч”Ч•Ч›ЧњЧ‘]?[\u0591-\u05C7]*)(Чђ[\u0591-\u05C7]*Ч“[\u0591-\u05C7]*Ч [\u0591-\u05B3\u05B5-\u05C7]*Ч™[\u0591-\u05B3\u05B5-\u05C2\u05C4-\u05C7]*|ЧђЧ“Ч•Ч©Чќ)(?=[<\[\(\s.,;:Чѓ'"\-]|$)/g;

  // only allow segol or tzere nikkud, so doesn't match ЧђЦІЧњЦµЧ”Ц¶Ц–Чќ or the like
  const elokaiRE = /([\s.,\u05BE;:'"\-]|^)([Ч•]?[\u0591-\u05C7]*[ЧћЧ©Ч”Ч•Ч›ЧњЧ‘]?[\u0591-\u05C7]*)(Чђ[\u0591-\u05AF\u05B1\u05B5\u05B6\u05BC-\u05C7]*Чњ[\u0591-\u05C7]*Ч•?[\u0591-\u05C7]*)([Ч”Ч§])([\u0591-\u05C7]*)((Ч™[\u0591-\u05C2\u05C4-\u05C7]*)?[ЧљЧ™Ч•Ч”ЧќЧџ][\u0591-\u05C2\u05C4-\u05C7]*|(Ч™[\u0591-\u05C7]*)?Ч [\u0591-\u05C7]*Ч•[\u0591-\u05C7]*|(Ч™[\u0591-\u05C7]*)?Ч›[[\u0591-\u05C2\u05C4-\u05C7]*[ЧќЧџ])(?=[\s<\[\(.,;Чѓ:'"\-]|$)/g;

  const elokaRE = /([\s.,\u05BE;:'"\-]|^)([Ч•]?[\u0591-\u05C7]*[ЧћЧ©Ч”Ч•Ч›ЧњЧ‘]?[\u0591-\u05C7]*)(Чђ[\u0591-\u05AF\u05B1\u05B5\u05B6\u05BC-\u05C7]*Чњ[\u0591-\u05C7]*Ч•[\u0591-\u05C7]*)([Ч”Ч§])([\u0591-\u05C2\u05C4-\u05C7]*)(?=[)(?=[\s<\[\(.,;:Чѓ'"\-]|$)/g;

  // const shadaiRE  = /([\s.,\u05BE;:'"\-]|^)([ЧћЧ©Ч”Ч•Ч›ЧњЧ‘]?[\u0591-\u05C7]*)(Ч©[\u0591-\u05C7]*[Ч“Ч§][\u0591-\u05C7]*Ч™[\u0591-\u05C7]*)(?=[\s.,;Чѓ:'"\-]|$)/g;
  ...
  const adoshemSub = divineName == "noSub" ? "ЧђЧ“Ч Ч™" : "ЧђЧ“Ч•Ч©Чќ";
  const elokaiSub = divineName == "noSub" ? "Ч”" : "Ч§";
    ...
    >
      ...
      {linkPopoverOpen ? (
        <div className="popup" contentEditable={false} onFocus={() => setEditingUrl(true)} onBlur={(e) => closePopup(e)}>
          ...
          />
          {showLinkRemoveButton ? <button onClick={() => xClicked()}>вњ•</button> : null}
        </div>
      ) : null}
