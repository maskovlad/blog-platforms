...
export async function deletePost(
  ...
  try {
    ...
    if (response?.site?.subdomain) {
      ...
      await revalidate(
        `${process.env.NEXT_PUBLIC_SITE_PROTOCOL}${response.site?.subdomain}.${process.env.NEXT_PUBLIC_SITE_URL}`, // hostname to be revalidated
...
export async function updatePost(
  ...
  try {
    ...
    if (subdomain) {
      // revalidate for subdomain
      await revalidate(
        `${process.env.NEXT_PUBLIC_SITE_PROTOCOL}${subdomain}.${process.env.NEXT_PUBLIC_SITE_URL}`, // hostname to be revalidated
        subdomain, // siteId
